...
public class RpcFunctionsServer {
	...
	public RpcAnswer Msg(RpcMessage message) {
		...
		if (message instanceof RpcMessage_GetExportList) {
			...
			// geladene CacheList zur Liste der gespeicherten CacheLists hinzuf�gen
			loadedCacheLists.put(msg.getCategoryId(), loadedCacheList);
			//xx			} else {
			// CacheList m�sste bereits geladen sein -> nur noch daraus die entsprechenden Caches �bertragen
			...
			// TODO Antwort kann Informationen �ber Konflikte �bergeben, ebenfalls als exportList
			// f�r jeden Konflikt wird ein Eintrag (ExportEntry) in dieser Liste �bergeben
