...
public class GetExportListDao {
	...
	public RpcAnswer getList() {
		...
		try {
			CoreCursor reader = Database.Data.db.rawQuery("SELECT gpx.CategoryId, cat.GpxFilename, count(*) from Caches c INNER JOIN GPXFilenames gpx, Category cat on c.GpxFilename_Id=gpx.Id AND gpx.CategoryId=cat.Id GROUP BY gpx.CategoryId", null);
